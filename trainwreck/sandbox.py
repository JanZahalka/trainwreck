...
class PoisonedGTSRB(torchvision.datasets.GTSRB):
    def set_image_data(self, i, new_image):
        ...
        self._samples[i] = (new_image, self._samples[i][1])
        print(self._samples[i])

    def set_image_label(self, i, new_label):
        print(self._samples[i])
        self._samples[i] = (self._samples[i][0], new_label)
        print(self._samples[i])

    def swap_items(self, i1, i2):
        i1_old = copy.deepcopy(self._samples[i1])

        self._samples[i1] = self._samples[i2]
        self._samples[i2] = i1_old
